...
// Modify with your DB configuration
...
async function runScript() {
  try {
    // Insert data from cars array
    for (const car of cars) {
      const { insertId } = await query("INSERT INTO Cars SET ?", {
        model: car.model,
        price: car.price,
      ...
      const carId = insertId;

      for (const option of car.options) {
        const { insertId } = await query("INSERT INTO Options SET ?", {
          option_description: option,
        });
        const optionId = insertId;
        await query("INSERT INTO CarOptions SET ?", {
          car_id: carId,
          option_id: optionId,
        });
      }

      for (const img of car.imgUrl) {
        await query("INSERT INTO Images SET ?", { car_id: carId, imgUrl: img });
      }
    }
